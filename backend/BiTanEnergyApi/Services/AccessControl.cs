...
public static class AccessControl
{
    ...
    // 使用者不用重新登入就會立刻生效。
    // 管理員預設不受限；但如果管理員帳號也勾選了可管理門市，就跟一般使用者一樣只限定在那些門市。
    public static async Task<List<string>?> GetAllowedGroupsAsync(ClaimsPrincipal user, MongoContext db)
    {
        ...
        if (string.IsNullOrEmpty(uid)) return isAdmin ? null : new List<string>();

        var account = await db.AdminUsers.Find(u => u.Id == uid).FirstOrDefaultAsync();
        if (isAdmin)
            return account != null && account.AssignedGroups.Count > 0 ? account.AssignedGroups : null;
